...
open class BMPlayerLayerView: UIView {
    ...
    /// 播放器的几种状态
    open var state = BMPlayerState.notSetURL {
        didSet {
            if state != oldValue {
    ...
    override open func layoutSubviews() {
        ...
        print("我进来了")
        guard let presentationSize = self.playerItem?.presentationSize, presentationSize != .zero else {return}
        let viewSize = self.bounds
        
        var size: CGSize = .zero
        let widthScale = presentationSize.width / viewSize.width
        let heightScale = presentationSize.height / viewSize.height
        if self.aspectRatio == .default {
            if widthScale < heightScale {
                size = CGSize(width: viewSize.height * presentationSize.width / presentationSize.height, height: viewSize.height)
            } else {
                size = CGSize(width: viewSize.width, height: viewSize.width * presentationSize.height / presentationSize.width)
            }
        } else {
            if widthScale < heightScale {
                size = CGSize(width: viewSize.width, height: viewSize.width * presentationSize.height / presentationSize.width)
        ...
        self.playerLayer?.frame = .init(x: (self.bounds.width - size.width) * 0.5, y: (self.bounds.height - size.height) * 0.5, width: size.width, height: size.height)
    }
